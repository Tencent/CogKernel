...
def extract_action_for_web(current_accessbility_tree, raw_action_code, expanded_part):
    ...
    if (
        ...
        if expanded_part and int(target_id) in expanded_part:
            ...
            print ("Expanded target found", expand_target_id, expand_target_type, 
                   expand_target_name, target_element_name)
            return ({
                "action_name": 'select',
...
system_message = """You are an autonomous intelligent agent tasked with navigating a web browser. 
...
Thought: {Your brief thoughts (briefly summarize the info that will help complete the task)} Action: 
```{the next action you choose to take}```"""
...
def call_web(
    ...
    while get_accessibility_tree_succeed:
        ...
        if counter > max_steps:
            break
        if action_error:
            current_user_message = """The action you have chosen cannot be executed. 
            Please double-check if you have selected the correct element or used correct action format. 
        ...
        if extracted_action["action_name"] is None:
            ...
            if not action_succeed:
                ...
                default_name = 'downloaded the following files:'
                if default_name in current_accessbility_tree:
                    path_str = current_accessbility_tree.split(default_name)[1].strip().split('OBJECTIVE:')[0]
                ...
                current_accessbility_tree, expanded_part = check_if_menu_is_expanded(current_accessbility_tree, 
                                                                                     snapshot)
    ...
    for i, line in enumerate(lines):
        ...
        if i in node_to_expand:
            for child in node_to_expand[i][1]:
                child_content = f"{child.get('role', '')} '{child.get('name', '')}' " + \
                ' '.join([f"{k}: {v}" for k, v in child.items() if k not in ['role', 'name']])
