...
class CognitiveKernel(object):
    ...
    def setup_global_db(self):
        for tmp_character in self.global_db_info:
            for tmp_db in self.global_db_info[tmp_character]:
                ...
                self.knowledge_engine.setup_global_ke_module(tmp_config)
    ...
    def _get_history_db_connection(self, CKStatus):
        ...
        target_ke_module = (
            self.knowledge_engine.get_local_ke_module_fresh(
    ...
    def retrieve_feedback(self, query, CKStatus):
        ...
        _, relevant_meta_data, _ = (
            current_db_connection.find_relevant_knowledge_single_d(
                query=query,
                retrieval_mode="doc",
                sim_threshold=0.5,
                soft_match_top_k=10,
    ...
    def _get_system_message(self, query, CKStatus):
        ...
        if ACTIVATE_HISTORY:
            ...
            system_message += "The following sentences are retrieved from the user's previous dialogs. "
            system_message += "If any of them are relevant to the user's current query "
            system_message += "then use them when responding to the user: "
...
def CallWeb(query, target_url):
    ...
    async def generate(
        ...
        """This function is the main generation function of cognitive kernel. 
        It will respond based on the current messages and role.
