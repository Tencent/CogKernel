...
class KnowledgeEngine(object):
    ...
    def setup_global_ke_module(self, config_file, mode="inference"):
    ...
    def get_local_knowledge_engine_module(self, config_file=None, module_name=None):
        """
        This function will create a local knowledge engine module. 
        If the config_file is a valid config file, we will create
    ...
    def find_relevant_info_single_db(
        ...
        if db_name in self.global_id2module:
            ...
            knowledge_engine_module = self.get_local_ke_module_fresh(
                module_name=db_name
            )
        ...
        if (
            ...
            docs_1, metadata_1, scores_1 = (
                knowledge_engine_module.find_relevant_knowledge_single_p(
                    query,
                    retrieval_mode,
                    hard_match_degree,
                    soft_match_top_k,
        ...
        if "doc" in retrieval_mode or "keyword" in retrieval_mode:
            docs_2, metadata_2, scores_2 = (
                knowledge_engine_module.find_relevant_knowledge_single_d(
...
class KnowledgeEngineModule(object):
    ...
    def _hard_retrieval_single_proposition(
        ...
        key_concept_and_perspectives = self._concept_perspective_gen_batch(
            [query]
        )
        if hard_match_degree == -1:
            ...
            target_pids_raw_data = (
                self.db_connection.get_rows_by_s_key_m_values(
    ...
    def _concept_retrieval_single_p(
        self, query, soft_match_top_k, sim_threshold, neighbor_size
    ):
        parse_query = False
        if parse_query:
            cp = self._concept_perspective_gen_batch([query])[0]
            if (
                cp is None
    ...
    def find_relevant_knowledge_single_d(
        ...
        document_contents, doc_metadata = (
            self.knowledge_engine_connection.retrieve_d_content_based_on_d_ids(
                retrieved_dids
            )
        )
        return document_contents, doc_metadata, scores

    def find_relevant_knowledge_single_p(
        self,
        query,
        retrieval_mode,
        hard_match_degree,
        ...
        with concurrent.futures.ThreadPoolExecutor() as executor:
            ...
            if "concept" in retrieval_mode:
                futures_concept_result = executor.submit(
                    self._concept_retrieval_single_p,
                    query,
                    soft_match_top_k,
                    sim_threshold,
                    neighbor_size,
        ...
        doc_contents, doc_metadata, doc_scores = (
            self.knowledge_engine_connection.retrieve_doc_content_based_on_p_ids(
    ...
    def _update_from_documents_by_batch(
        ...
        if (
            ...
            self.knowledge_engine_connection.insert_d_p_mappings(
            ...
            with concurrent.futures.ThreadPoolExecutor() as executor:
                futures_key_concept_and_perspectives = executor.submit(
                    self._concept_perspective_gen_batch, all_propositions
                )
    ...
    def _updating_embedding(self, ids):
        ...
        """
        existing_rows = self.db_connection.get_rows_by_s_key_m_values(
            table_name=PROPOSITION_TABLE_NAME,
            by="_id",
            keys=ids,
    ...
    def _concept_perspective_gen_batch(self, input_queries):
        """
    ...
    def _proposition_generation_batch(self, input_queries):
        ...
        for tmp_query in input_queries:
            model_inputs.append(
                "将“"
                + tmp_query
                + '”拆解成更细粒度的短句，每个短句应该包括完整的信息。'
                + '简单来说，一个短句就是一个你能够通过查询来证明，证伪或回答的点。所有短句都应该是基于原句的改写，不要创造内容。要保证每个语义单元信息完整，语法通顺。'
                + '每个短句中不应包含指代词。缺失的指代词要补成对应的实体或事件。要保证每个短句不包含分号等排比结构以及"和"，"或"等连接词。以json格式输出。以列表输出。'
            )
        inference_results = call_tgi_server_batch_parallel(
